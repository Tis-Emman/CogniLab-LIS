...
export default function UserManagementPage() {
  ...
  const validateForm = () => {
    ...
    const memberCount = users.filter(
      (u) => u.role === "member" && u.id !== editingId,
    ).length;
    const facultyCount = users.filter(
      (u) => u.role === "faculty" && u.id !== editingId,
    ).length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Manage system users and access permissions (9 total: 8 members + 1
            faculty)
          </p>
        </div>
        <button
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      role: e.target.value as "member" | "faculty",
                    })
                  }
                  className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#3B6255] text-gray-800 bg-white ${
