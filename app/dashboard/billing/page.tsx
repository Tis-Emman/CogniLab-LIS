...
export default function BillingPage() {
  ...
  const handlePaymentStatusChange = async (id: string, newStatus: 'paid' | 'unpaid') => {
    ...
    if (newStatus === 'paid' && billing) {
      // Auto-create specimen_tracking record if one doesn't exist yet
      ...
      if (!existing) {
        await supabase.from('specimen_tracking').insert([{
          patient_name: billing.patient_name || billing.patientName || '',
          test_name:    billing.test_name    || billing.testName    || '',
    ...
    if (newStatus === 'unpaid') {
      // Remove specimen only if it hasn't progressed past pending_accession
      await supabase
        .from('specimen_tracking')
        .delete()
        .eq('billing_id', id)
        .eq('status', 'pending_accession');
    }
