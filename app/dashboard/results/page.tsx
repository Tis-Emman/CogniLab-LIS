          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center z-10">
                ...
                {isCurrent && (
                  <span className="text-[8px] text-[#3B6255] font-bold mt-0.5">
                    CURRENT
                  </span>
                )}
              </div>
...
export default function TestResultsPage() {
  ...
  const [expandedStepperId, setExpandedStepperId] = useState<string | null>(
    null,
  );
  ...
  const handlePrint = (result: TestResult) => {
    const isCbcComponent =
      result.section === "HEMATOLOGY" &&
      ["Neutrophils", "Lymphocytes", "Monocytes", "Eosinophils", "Basophils"].includes(result.test_name);
    const billingTestName = isCbcComponent ? "CBC" : result.test_name;
    const billing = billings.find(
      (b) => b.test_name === billingTestName && b.patient_name === result.patient_name,
    );
    if (!billing || billing.status !== "paid") {
      alert("⚠️ Cannot print report. This patient has an unpaid balance. Please settle the billing first.");
      return;
    }
  ...
  const moveToNextStatus = async (resultId: string, currentStatus: string) => {
    ...
    if (currentIndex < statusFlow.length - 1) {
      ...
      setResults(
        results.map((r) =>
          r.id === resultId
            ? { ...r, status: nextStatus as TestResult["status"] }
            : r,
        ),
      );
  ...
  const selectedPatient = patients.find(
    (p) => `${p.first_name} ${p.last_name}` === patientName,
  );
  ...
  const CC_HIDDEN_INDIVIDUAL = [
    ...
    "Oral Glucose Tolerance Test (OGTT) 100g",
    "Oral Glucose Tolerance Test (OGTT) 75g",
    "Oral Glucose Challenge Test (OGCT) 50g",
  ...
  const CC_GROUPED_ENTRIES = CC_GROUPED_TEST_NAMES.map((name) => ({
    name,
    referenceRange: "",
    unit: "",
  }));
    ...
    ? (() => {
        let base = (TESTS_BY_SECTION[selectedSection] || []).filter((t) => {
          if (
            ...
            HEMATOLOGY_HIDDEN.includes(t.name)
          )
            return false;
          if (
            selectedSection === "CLINICAL CHEMISTRY" &&
            CC_HIDDEN_INDIVIDUAL.includes(t.name)
          )
            return false;
          return true;
        });
    ...
    try {
      ...
      if (selectedTest === "CBC") {
        const cbcLines = [
          ...
          ...(cbcPeripheralSmearValue
            ? [`Peripheral Blood Smear: ${cbcPeripheralSmearValue}`]
            : []),
        ];
        const payload = {
        ...
        if (editingId) {
          // editingId points to whichever individual record was opened for edit.
          // Re-save that record's value from the matching rbcValues field,
          // then upsert the other two as new records so no value is silently dropped.
          const rbcComponents = [
            { name: "MCV", value: rbcValues.mcv, range: "80 - 100", unit: "fL" },
            { name: "MCH", value: rbcValues.mch, range: "27 - 31", unit: "pg" },
            { name: "RDW", value: rbcValues.rdw, range: "11.5 - 14.5", unit: "%" },
          ];
          // Update the record that was opened for editing first
          const editedComponent = rbcComponents.find((c) =>
            results.find((r) => r.id === editingId)?.test_name === c.name
          ) ?? rbcComponents[0];
          await updateTestResult(
            editingId,
            {
              patient_name: patientName,
              section: selectedSection,
              test_name: editedComponent.name,
              result_value: editedComponent.value,
              reference_range: editedComponent.range,
              unit: editedComponent.unit,
            },
            user,
          );
          // Upsert the remaining two components as new records
          for (const c of rbcComponents.filter((c) => c.name !== editedComponent.name)) {
            await addTestResult(
              {
                patient_name: patientName,
                section: selectedSection,
        ...
        const testName =
          sexLabel === "Male" ? "Hemoglobin (Male)" : "Hemoglobin (Female)";
        const referenceRange =
          sexLabel === "Male" ? "14.0 - 17.0" : "12.0 - 15.0";
        const payload = {
          patient_name: patientName,
          section: selectedSection,
          test_name: testName,
  ...
  const handleEdit = (result: TestResult) => {
    ...
    if (result.test_name === "CBC") {
      ...
      setCbcValues({
        neutrophils: num(kv["Neutrophils"]),
        lymphocytes: num(kv["Lymphocytes"]),
        monocytes: num(kv["Monocytes"]),
        eosinophils: num(kv["Eosinophils"]),
        basophils: num(kv["Basophils"]),
      });
      setCbcRbcValues({
        mcv: num(kv["MCV"]),
        mch: num(kv["MCH"]),
      ...
      setRbcValues((prev) => ({
        ...prev,
        [result.test_name.toLowerCase()]: num(raw),
      }));
      setResultValue(raw);
    } else if (
      result.test_name === "Hemoglobin (Male)" ||
      result.test_name === "Hemoglobin (Female)"
    ) {
      setSelectedTest("Hemoglobin");
      setHemoglobinValue(num(raw));
    } else if (result.test_name === "Routine Urinalysis (UA)") {
      ...
      setUrinalysisValues({
        ...
        wbcMicroscopic: num(kv["WBC (Microscopic)"]),
      ...
      setCultureSensitivityValues({
        culture: kv["Culture"] || "",
        preliminaryReport: kv["Preliminary Report"] || "",
        finalReport: kv["Final Report"] || "",
        sensitivity: kv["Sensitivity"] || "",
      });
    } else {
    ...
    <div className="space-y-8" style={{ animation: "fadeIn 0.5s ease-out" }}>
      ...
      {showForm && (
        <div
          ...
          style={{ animation: "fadeInSlideUp 0.6s ease-out 0.2s backwards" }}
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {/* Unpaid billing warning */}
              {patientName && (() => {
                const hasUnpaid = billings.some(
                  (b) => b.patient_name === patientName && (b.status || b.paymentStatus) !== "paid"
                );
                if (!hasUnpaid) return null;
                return (
                  <div className="mt-2 flex items-start gap-2 px-4 py-3 bg-red-50 border border-red-300 rounded-lg text-red-700 text-sm">
                    <span className="text-lg leading-none">⚠️</span>
                    <div>
                      <p className="font-semibold">Unpaid balance detected</p>
                      <p className="text-xs mt-0.5 text-red-500">This patient has an outstanding bill. Please settle the payment in Billing before encoding results.</p>
                    </div>
                  </div>
                );
              })()}
            ...
            <div>
              ...
              {selectedSection && (
                <div
                  className={`mt-3 p-3 rounded-lg text-sm border ${selectedSection === "CLINICAL CHEMISTRY" ? "bg-[#CBDED3] border-[#8BA49A] text-[#3B6255]" : "bg-yellow-50 border-yellow-300 text-yellow-800"}`}
                >
                  ...
                  {selectedSection === "CLINICAL CHEMISTRY" && (
                    <>
                      ...
                      <optgroup label="── Individual Tests ──">
                        {currentTests
                          .filter(
                            (t) => !CC_GROUPED_TEST_NAMES.includes(t.name),
                          )
                          .map((test) => (
              ...
              <div>
                ...
                {selectedTest === "CBC" && (
                  <div className="space-y-4">
                    ...
                    {errors.cbc && (
                      <p className="text-red-500 text-sm">{errors.cbc}</p>
                    )}
                    <div className="pt-3 border-t border-gray-200 space-y-3">
                      ...
                      {errors.rbc && (
                        <p className="text-red-500 text-sm">{errors.rbc}</p>
                      )}
                    </div>
                    <div className="pt-3 border-t border-gray-200 space-y-3">
                      <label className="block text-sm font-semibold text-gray-700">
                      ...
                      <input
                        type="text"
                        value={cbcHemoglobinValue}
                        onChange={(e) => setCbcHemoglobinValue(e.target.value)}
                        placeholder={
                          selectedPatientSex === "Female"
                            ? "12.0 - 15.0"
                            : "14.0 - 17.0"
                        }
                        className={inputCls()}
                    ...
                    <div className="pt-3 border-t border-gray-200 space-y-3">
                      ...
                      <input
                        type="text"
                        value={cbcHematocritValue}
                        onChange={(e) => setCbcHematocritValue(e.target.value)}
                        placeholder={
                          selectedPatientSex === "Female"
                            ? "37 - 47"
                            : "40 - 54"
                    ...
                    <div className="pt-3 border-t border-gray-200 space-y-3">
                      ...
                      <textarea
                        value={cbcPeripheralSmearValue}
                        onChange={(e) =>
                          setCbcPeripheralSmearValue(e.target.value)
                        }
                        placeholder="Enter findings (optional)"
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B6255] outline-none transition text-gray-800 placeholder-gray-500 bg-white min-h-[80px]"
                      />
                    </div>
                    <div className="pt-3 border-t border-gray-200 space-y-3">
                      <label className="block text-sm font-semibold text-gray-700">
                      ...
                      <input
                        type="text"
                        value={cbcPlateletCountValue}
                        onChange={(e) =>
                          setCbcPlateletCountValue(e.target.value)
                        }
                        placeholder="150 - 450"
                        className={inputCls()}
                      />
                ...
                {selectedTest === "RBC Indices (MCV, MCH, RDW)" && (
                  <div className="space-y-4">
                    <label className="block text-sm font-semibold text-gray-700">
                      RBC Indices <span className="text-red-500">*</span>
                    </label>
                    {[
                      {
                        label: "MCV (fL)",
                ...
                {selectedTest === "Hemoglobin" && (
                  <div className="space-y-3">
                    <label className="block text-sm font-semibold text-gray-700">
                      Hemoglobin ({selectedPatientSex || "Male"}){" "}
                      <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      ...
                      className={inputCls(errors.hemoglobin)}
                    />
                    <p className="text-xs text-gray-500">
                      Ref:{" "}
                      {selectedPatientSex === "Female"
                ...
                {selectedTest === "PT/INR, PTT" && (
                  <div className="space-y-4">
                    ...
                    {[
                      {
                        label: "Prothrombin Time (PT) (seconds)",
                        key: "pt",
                        ph: "11.0 - 13.5",
                      ...
                      <div key={key}>
                        ...
                        <input
                          type="text"
                          value={(coagulationValues as any)[key]}
                          onChange={(e) =>
                            setCoagulationValues({
                              ...coagulationValues,
                              [key]: e.target.value,
                ...
                {selectedTest === "Arterial Blood Gas" && (
                  <div className="space-y-4">
                    ...
                    {errors.abg && (
                      <p className="text-red-500 text-sm">{errors.abg}</p>
                    )}
                  </div>
                )}
                    ...
                    <div className="flex gap-2">
                      {TEST_DROPDOWN_OPTIONS[selectedTest] ? (
                        <select
                          ...
                          className={`flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#3B6255] outline-none transition text-gray-800 bg-white ${errors.resultValue ? "border-red-500" : "border-gray-300"}`}
                        >
                          <option value="">Select result</option>
                          {TEST_DROPDOWN_OPTIONS[selectedTest].map((o) => (
                            <option key={o} value={o}>
                              {o}
                            </option>
                          ))}
                        ...
                        <input
                          ...
                          className={`flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#3B6255] outline-none transition text-gray-800 placeholder-gray-500 bg-white ${errors.resultValue ? "border-red-500" : "border-gray-300"}`}
                        />
                      )}
            ...
            <div className="flex gap-4 pt-6 border-t border-gray-200">
              {(() => {
                const isUnpaid = !!patientName && billings.some(
                  (b) => b.patient_name === patientName && (b.status || b.paymentStatus) !== "paid"
                );
                return (
                  <button
                    type="submit"
                    disabled={submitting || isUnpaid}
                    title={isUnpaid ? "Patient has an unpaid balance — settle billing first" : undefined}
                    className={"flex-1 px-6 py-3 rounded-lg transition font-semibold " + (isUnpaid ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-gradient-to-r from-[#3B6255] to-green-900 text-white hover:shadow-lg disabled:opacity-50")}
                  >
                    {submitting
                      ? "Saving..."
                      : isUnpaid
                        ? "⚠️ Unpaid — Cannot Save"
                        : editingId
                          ? "✓ Update Result"
                          : "✓ Save Test Result"}
                  </button>
      ...
      <div
        ...
        style={{ animation: "fadeInSlideUp 0.6s ease-out 0.3s backwards" }}
      >
        <h2 className="text-2xl font-bold text-gray-800 mb-6">
          Available Laboratory Sections
        </h2>
      ...
      <div
        ...
        style={{ animation: "fadeInSlideUp 0.6s ease-out 0.5s backwards" }}
      >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative">
                <input
                  ...
                  className="w-full sm:w-64 px-4 py-2 pl-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B6255] outline-none transition text-gray-800 placeholder-gray-500 bg-white"
                />
                <svg
              ...
              <select
                ...
                className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B6255] outline-none transition text-gray-800 bg-white"
              >
              ...
              {results
                .filter((result) => {
                  if (searchTerm) {
                    ...
                    if (
                      !result.patient_name?.toLowerCase().includes(s) &&
                      !result.test_name?.toLowerCase().includes(s) &&
                      !result.section?.toLowerCase().includes(s)
                    )
                      return false;
                  }
                ...
                .map((result) => {
                  const isCbcComponent =
                    ...
                    [
                      ...
                      "Eosinophils",
                      "Basophils",
                    ].includes(result.test_name);
                  const billingTestName = isCbcComponent
                    ? "CBC"
                    : result.test_name;
                  ...
                  const abnormal = getAbnormalStatus(
                    result.result_value,
                    result.test_name,
                    result.section,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-8 relative">
                        <button
                          onClick={() =>
                            setExpandedStepperId(
                              expandedStepperId === result.id
                                ? null
                                : result.id,
                            )
                          }
                        ...
                        >
                          {updatingStatusId === result.id
                            ? "Updating..."
                            : getStatusLabel(result.status)}
                          <ChevronRight
                            className={`w-3 h-3 transition-transform ${expandedStepperId === result.id ? "rotate-90" : ""}`}
                          />
                        </button>
                      </td>
                      ...
                      <td className="py-4 px-8">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1 w-fit ${billing?.status === "paid" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                        >
                          {billing?.status === "paid" ? (
                            <>✓ Paid</>
                          ) : (
                      ...
                      <td className="py-4 px-8 flex gap-2">
                        ...
                        {result.status === "released" && (
                          billing?.status === "paid" ? (
                            ...
                            <button
                              disabled
                              className="text-gray-400 font-semibold text-sm flex items-center gap-1 cursor-not-allowed"
                              title="⚠️ Unpaid — settle billing before printing"
                            >
                              <Printer className="w-4 h-4" />
                              Print
                            </button>
      ...
      {printResult &&
        (() => {
          ...
          const price = getTestPrice(
            printResult.test_name,
            printResult.section,
          );
          const abnormalStatus = getAbnormalStatus(
              ...
              >
                ...
                >
                  ...
                  <p style={{ margin: "0", fontSize: "12px", color: "#666" }}>
                    Professional Laboratory Information System
                  </p>
                </div>
                <div style={{ marginBottom: "30px" }}>
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {new Date(printResult.date_created).toLocaleDateString(
                          "en-US",
                  ...
                  <div style={{ fontSize: "13px" }}>
                    ...
                    <div
                      style={{
                        ...
                        justifyContent: "space-between",
                        fontWeight: "bold",
                        color: billing?.status === "paid" ? "green" : "orange",
                      }}
                    >
                ...
                >
                  <p style={{ margin: "5px 0" }}>
                    This is an official test result from the Laboratory
                    Information System
                  </p>
                  <p style={{ margin: "5px 0" }}>
